...
- 可选强化学习命令行：9×9 自我对弈、MCTS、策略/价值网络训练、换色评测、
  固定局面与历史模型评测、棋谱导出和完整断点续训；训练对手包含冻结冠军与历史候选，
  正常对局期间自动暂停训练
- 实时训练监控网页：逐局胜率、当前对手与冠军版本、训练阶段、历史曲线和评测结果；
  Windows 运行 `py monitor_training.py`，打开 <http://127.0.0.1:8766>
...
训练监控可双击 `启动训练监控.bat`，或在项目目录执行：

```powershell
py monitor_training.py
# 默认查看某一份训练记录：
py monitor_training.py --run balanced_20260920
```

打开 <http://127.0.0.1:8766>。网页每 2 秒自动刷新，运行目录可在页面切换；
服务只读取训练数据，无需 PyTorch，也不会自动开始训练。
详情见 [训练监控说明](docs/RL_TRAINING.md#实时监控网页)。
